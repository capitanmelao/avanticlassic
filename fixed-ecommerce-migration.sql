-- ===================================================================
-- FIXED E-COMMERCE MIGRATION FOR AVANTI CLASSIC
-- Removed broken RLS policies that reference non-existent auth tables
-- Uses simple security model compatible with custom authentication
-- Date: July 17, 2025
-- ===================================================================
...
-- ===================================================================
-- STEP 8: SIMPLE SECURITY MODEL (NO COMPLEX RLS)
-- ===================================================================

-- Note: Since the admin panel uses custom authentication with HTTP-only cookies,
-- we don't use RLS policies that reference Supabase Auth tables.
-- Security is handled at the application level through the custom auth system.

-- Basic public read access for the main site (no authentication required)
-- This allows the main site to display products publicly

-- Grant public read access to product data
GRANT SELECT ON public.products TO anon;
GRANT SELECT ON public.product_prices TO anon;
GRANT SELECT ON public.product_categories TO anon;
GRANT SELECT ON public.product_category_relations TO anon;

-- Grant authenticated users access to cart and orders
GRANT SELECT, INSERT, UPDATE, DELETE ON public.cart_items TO authenticated;
GRANT SELECT, INSERT ON public.orders TO authenticated;
GRANT SELECT ON public.order_items TO authenticated;
GRANT SELECT, INSERT ON public.customers TO authenticated;

-- Admin panel access is controlled by the custom authentication system
...
BEGIN
    ...
    RAISE NOTICE '=== FIXED E-COMMERCE MIGRATION COMPLETED ===';
    ...
    RAISE NOTICE '- No RLS policies (compatible with custom auth)';
    RAISE NOTICE '- Application-level security via HTTP-only cookies';
    RAISE NOTICE '- Public read access for main site product display';
    RAISE NOTICE '- Admin access controlled by custom authentication';
END $$;

-- Migration completed successfully with proper authentication compatibility!